...
from tests.testing_processor import fetch_events, fetch_ordered_spans, fetch_traces
...
class _Workflow(VoiceWorkflowBase):
    async def run(self, transcription: str) -> AsyncIterator[str]:
        ...
        with custom_span("workflow", data={"transcript": transcription}):
            yield _TEXT
...
async def test_disabled_voice_pipeline_preserves_caller_lifecycle(
    ...
    with trace("caller") if outcome != "success" else nullcontext():
    ...
    assert {item["span_data"].get("name") for item in spans} == (
        {"concurrent sibling", "after sibling"} if caller is not None else set()
    )
    if caller is not None:

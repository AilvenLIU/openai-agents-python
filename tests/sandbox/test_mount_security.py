...
def test_builtin_mount_subclass_is_rejected_by_execution_provenance() -> None:
    ...
    sanitized, redacted = sanitize_manifest_mount_authority(external)

    assert redacted is True
    assert "example-secret-key" not in repr(sanitized)
    sanitized_mount = sanitized.entries["data"]
    assert isinstance(sanitized_mount, CustomS3Mount)
    assert sanitized_mount.access_key_id is None
    assert sanitized_mount.secret_access_key is None
    assert sanitized_mount.api_token is None
    assert "custom-mount-secret" not in repr(sanitized)
...
def test_session_state_serialization_redacts_custom_strategy_with_known_discriminator() -> None:
    ...
    serialized_strategy = payload["manifest"]["entries"]["data"]["mount_strategy"]  # type: ignore[index]
    assert payload[REDACTED_MOUNT_AUTHORITY_KEY] is True
    assert serialized_strategy == {"type": "docker_volume"}
    assert "custom-strategy-secret" not in repr(payload)

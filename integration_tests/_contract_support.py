...
class _ModuleBindingVisitor(ast.NodeVisitor):
    ...
    def visit_ImportFrom(self, node: ast.ImportFrom) -> None:
        if self._bindings_target_module:
            self.has_wildcard_import |= any(imported.name == "*" for imported in node.names)
        for imported in node.names:
            if imported.name != "*":
...
def _binding_occurrences(module_tree: ast.Module, name: str) -> tuple[int, bool]:
    visitor = _ModuleBindingVisitor(name)
    visitor.visit(module_tree)
    return visitor.count, visitor.has_wildcard_import
...
def _direct_import_source(
    ...
    import_sources: list[tuple[str, str]] = []
    for statement in module_tree.body:
        ...
        for imported_name in statement.names:
            ...
            if binding_name != export_name or imported_name.name == "*":
                continue
            if statement.level:
            ...
            if source_module_name is None or not (
                source_module_name == package_root
                or source_module_name.startswith(f"{package_root}.")
            ):
                continue
            import_sources.append((source_module_name, imported_name.name))

    binding_count, has_wildcard_import = _binding_occurrences(module_tree, export_name)
    if len(import_sources) != 1 or binding_count != 1 or has_wildcard_import:
        return None
    import_source = import_sources[0]
    source_module = sys.modules.get(import_source[0])
    if not isinstance(source_module, ModuleType):
        return None
    return source_module, import_source[1]
...
def _type_alias_definition(
    ...
    if _is_type_alias_type(value):
        ...
        return _type_alias_definition(
            value.__value__, visited_alias_ids=visited_alias_ids | {alias_id}
        )

...
def load_submodule_export_policy(path: Path) -> SubmoduleExportPolicy:
    ...
    for module_name, installation in dependencies.items():
        ...
        unknown_fields = sorted(set(installation) - {"extra", "requirement"})
        if unknown_fields:
            raise ValueError(
        ...
        if field_name == "extra":
            dependency_extras.append(install_value)
        else:
            dependency_requirements.append(install_value)
    ...
    return SubmoduleExportPolicy(
        modules=policy,
        dependency_extras=tuple(sorted(set(dependency_extras))),
        dependency_requirements=tuple(sorted(set(dependency_requirements))),
    )
...
def build_released_api_contract(
    ...
    *,
    ...
    if submodule_export_policy is not None:
        unknown_policy_modules = sorted(
            set(submodule_export_policy) - set(contract["public_modules"])
        )
        if unknown_policy_modules:
            raise ValueError(
                "submodule export policy contains modules outside public_modules: "
                f"{unknown_policy_modules!r}"
            )
...
def validate_released_api_contract(
    ...
    *,
    agents_module: Any | None = None,
    require_all_optional_dependencies: bool = False,
) -> list[str]:
    agents = agents_module or importlib.import_module("agents")
    errors: list[str] = []
    ...
    for module_name, released in contract.get("required_submodule_exports", {}).items():
        ...
        if require_all_optional_dependencies and (
            unavailable_optional_bindings or unavailable_optional_exports
        ):
            optional_dependencies = optional_bindings | optional_exports
            unavailable = sorted(
                f"{name} -> {optional_dependencies[name]}" for name in unavailable_optional_bindings
            )
            errors.append(

...
async def test_runner_owned_local_sandbox_cannot_inspect_trusted_client_credential(
    ...
    assert "--- environment ---" in model_requests
    assert "--- filesystem ---" in model_requests
    assert "--- processes ---" in model_requests
    assert not any(
    ...
    if result is not None:
        ...
        assert "--- environment ---" in str(tool_outputs[0])
        assert "--- filesystem ---" in str(tool_outputs[0])
        assert "--- processes ---" in str(tool_outputs[0])

    ...
    async def _run_compaction_locked(
        ...
        *,
        ...
        if not is_automatic and self.max_rollback_items is not None:
            # Check before even a default-unlimited candidate read. Keep the complete
            # snapshot under the wrapper mutation lock through replacement and rollback.
        ...
        if snapshot is None and previous_items is None and self.max_rollback_items is not None:
            previous_items = await self._get_all_underlying_session_items()

...
def _mount_class_is_trusted(mount: Mount) -> bool:
    ...
    if canonical_class is not None:
        return type(mount) is canonical_class
    return type(mount) is _trusted_extension_mount_class(mount.type)
...
def _pattern_class_is_trusted(pattern: object) -> bool:
    ...
    if not isinstance(pattern_type, str):
        return False
    return type(pattern) is _SERIALIZED_PATTERN_CLASS_BY_TYPE.get(pattern_type)
...
def _manifest_has_configured_mount_authority(manifest: Manifest) -> bool:
    ...
    while pending:
        entry = pending.pop()
        if isinstance(entry, Mount) and _configured_mount_authority_fields(entry):
            return True
    ...
    try:
        for value in (*args, *kwargs.values()):
            ...
            for candidate in candidates:
                ...
                if callable(has_runtime_authority) and has_runtime_authority():
                    return True
                if isinstance(candidate, Mount):
                    if _configured_mount_authority_fields(candidate):
                        return True
                    continue
                manifest = (
                    candidate
...
def _strategy_classification(strategy: MountStrategyBase) -> tuple[str, str | None]:
    classification = _STRATEGY_CLASSIFICATION_BY_TYPE.get(strategy.type)
    if classification is None:
        return "unknown", None
    boundary, backend_id, _module_name, _class_name = classification
    if type(strategy) is not _trusted_strategy_class(strategy.type):
        return "unknown", None
    ...
    for mount, _mount_path in manifest.mount_targets():
        if not _mount_class_is_trusted(mount):
            return MountConfigError(
                message=(
                    "custom mount implementations are not supported at the sandbox credential "
                    "boundary"
                )
            )
        mount_type = _canonical_mount_type(type(mount)) or mount.type
        ...
        if pattern is not None and not _pattern_class_is_trusted(pattern):
            return MountConfigError(
                message=(
                    "custom mount patterns are not supported at the sandbox credential boundary"
                )
            )
...
def rebind_manifest_mount_authority(
    ...
    *,
    ...
    sanitized_trusted, trusted_has_authority = sanitize_manifest_mount_authority(trusted_manifest)
    persisted_mounts = {
        path.as_posix(): entry
    ...
    if error is None and (not trusted_has_authority or not topology_matches):
        error = MountConfigError(
            message=(
                "sandbox mount authority can be rebound only from a current trusted external "
...
def _sanitize_raw_mount(
    ...
    elif entry_class is not None and issubclass(entry_class, Mount):
        configured_custom_fields = tuple(
            name
            for name, value in entry.items()
            if name not in Mount.model_fields and value is not None
        )
        if configured_custom_fields:
            error = _InvalidRawMountManifestError(
                "custom mount configuration cannot be restored safely"
            )
            _mark_error_data_redacted(error)
            _raise_data_redacted_error(error)
        safe_mount_fields = Mount.model_fields

...
class UnixLocalSandboxSession(BaseSandboxSession):
    ...
    _inherit_host_environment: bool
    ...
    def __init__(self, *, state: UnixLocalSandboxSessionState) -> None:
        ...
        self._inherit_host_environment = False
    ...
    async def _resolved_exec_context(self) -> tuple[dict[str, str], str]:
        if self._inherit_host_environment:
            env = dict(os.environ)
        else:
            env = {
                name: value
...
class UnixLocalSandboxClient(BaseSandboxClient[UnixLocalSandboxClientOptions | None]):
    ...
    def __init__(
        ...
        *,
        ...
        self._inherit_host_environment = inherit_host_environment
    ...
    async def create(
        ...
        *,
        ...
        # Keep full host inheritance under trusted runtime control. Session state and manifests
        # must not be able to enable it when a session is resumed by another client.
        inner._inherit_host_environment = self._inherit_host_environment

...
class _MCPServerWithClientSession(MCPServer, abc.ABC):
    ...
    def _safe_transport_cause(http_error: Exception) -> Exception | None:
        """Keep a transport cause only when its request URLs need no sanitization."""
        ...
        return (
            http_error if all(get_mcp_server_log_name(url) == url for url in request_urls) else None
        )
    ...
    async def connect(self):
        ...
        try:
            ...
            if http_error:
                ...
                connection_cause = self._safe_transport_cause(http_error)
            ...
            # For HTTP-related errors, wrap them
                ...
                connection_cause = self._safe_transport_cause(e)
    ...
    async def list_tools(
        ...
        try:
            ...
            transport_cause = self._safe_transport_cause(e)
            ...
            try:
                ...
                if http_error:
                    if is_failed_connection_cleanup:
                        ...
                        cleanup_cause = self._safe_transport_cause(http_error)
                    else:
                    ...
                    if is_failed_connection_cleanup:
                        ...
                        cleanup_cause = self._safe_transport_cause(connect_error)
                    else:
                        log_tool_action_warning(
                            logger,
                            get_mcp_server_log_message(
                    ...
                    if is_failed_connection_cleanup:
                        ...
                        cleanup_cause = self._safe_transport_cause(timeout_error)
                    else:
                        log_tool_action_warning(
                            logger,
                            get_mcp_server_log_message(
    ...
    async def call_tool(
        ...
        try:
            ...
            if isinstance(http_error, httpx.HTTPStatusError):
                ...
                raise
            transport_cause = self._safe_transport_cause(http_error)

...
async def run_single_turn_streamed(
    ...
    validated_model_items: list[RunItem] | None = None
    ...
    async def after_invocation_validation(
        ...
        nonlocal validated_model_items
        validated_model_items = model_items
        await gather_with_cancel(
            (
    ...
    async def emit_validated_model_items_before_side_effects() -> None:
        await raise_if_input_guardrail_tripwire_known()
        if validated_model_items is not None:
            emitted_model_item_occurrence_keys.update(
                _ensure_stream_event_item_occurrence_key(item) for item in validated_model_items
            )
            stream_step_items_to_queue(validated_model_items, streamed_result._event_queue)
    ...
    single_step_result = await get_single_step_result_from_response(
        ...
        before_side_effects=emit_validated_model_items_before_side_effects,
    )

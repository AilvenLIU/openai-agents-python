...
class ModelSettings:
    ...
    When enabled, ``ModelResponse.raw_usage`` contains a JSON-compatible snapshot captured
    before the Agents SDK normalizes missing usage fields. This setting does not request usage
    from the provider; use ``include_usage`` separately when a streaming provider requires it.
    """
